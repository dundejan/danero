...
export interface SaleSimulationResult {
  ...
  deltas: { taxCzk: Money; flatTax50kUsedCzk: Money; limit100kUsedCzk: Money };
}
...
export function simulateSale(
  ...
  return {
    baseline,
    simulated,
    simulatedDisposal: simulatedResult.securities.disposals.find(
      (report) => report.sellTxId === SIMULATED_SELL_ID,
    ),
    deltas: {

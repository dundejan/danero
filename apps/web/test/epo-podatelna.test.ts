...
describe('report: stránkování tabulky prodejů (G-P2 / H2-04)', () => {
  it('rozdělí prodeje po stranách, žádný nevynechá ani nezdvojí', async () => {
    const { disposalPage, DISPOSALS_PER_PAGE } = await import(
    ...
    for (let strana = 1; strana <= 10; strana += 1) {
      const { totalPages, currentPage, fromRow } = disposalPage(celkem, strana);
      stran = totalPages;
      if (strana > totalPages) break;
      expect(currentPage).toBe(strana);
      for (let i = fromRow; i < Math.min(fromRow + DISPOSALS_PER_PAGE, celkem); i += 1) {
        videne.push(i);
      }
    }
  ...
  it('strana mimo rozsah se ořízne, tabulka nikdy nevyjde prázdná', async () => {
    const { disposalPage } = await import('@/components/views/report-view');
    expect(disposalPage(450, 99).currentPage).toBe(3);
    expect(disposalPage(450, 0).currentPage).toBe(1);
    expect(disposalPage(450, -5).currentPage).toBe(1);
    expect(disposalPage(0, 1)).toEqual({ totalPages: 1, currentPage: 1, fromRow: 0 });
  });
});
